...
| **Mobile (< 768px)**           | Starts in "preview" state with heavy borders showing limited content (`inset-8`).                                 | Tapping the container expands it to near-fullscreen (`inset-1 z-50`). A full-width footer bar with a chevron icon and "Close" label sits at the bottom of the content area (not floating), acting as the collapse button. | Hidden while expanded; visible when collapsed.                       |
